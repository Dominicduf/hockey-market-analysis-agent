...
> Le projet a principalement été testé avec le modèle **`nvidia/nemotron-3-super-120b-a12b:free`**, déjà configuré par défaut dans `.env.example`. Il a aussi été testé avec **`openai/gpt-oss-120b:free`**, mais la performance sur les tools calls n'étaient pas au rendez-vous. Techniquement, le project devrait fonctionner sur tout modèle qui supporte le tool calling.
...
Le modèle `nvidia/nemotron-3-super-120b-a12b:free` peut occasionnellement échouer à retourner l'analyse de sentiment au format JSON attendu, entraînant une erreur lors de la génération du rapport. Relancer simplement l'analyse règle le problème dans la grande majorité des cas. Le modèle est aussi assez lent, donc l'analyse peut prendre quelques minutes. L'utilisation d'un modèle plus performant éliminerait cette instabilité.
...
- **report_generator** : génère un rapport PDF (ReportLab) contenant un graphique à barres des scores de sentiment (matplotlib) et un résumé textuel par produit. 
...
- **Grounding** : un effort de prompt engineering a été consacré à contraindre l'agent à ne produire aucune analyse basée sur ses connaissances internes, mais uniquement sur les données retournées par les outils.
...
- **Gestion des erreurs dans les outils** : au lieu de lever des exceptions, chaque outil retourne une chaîne préfixée `[TOOL ERROR]`, ce qui permet à l'agent de gérer l'échec d'en informer l'utilisateur.
...
product_sentiments (id, analysis_id → analyses, product_name, overall_score,
                    summary TEXT)
...
Coûts LLM: Utiliser un modèle moins coûteux pour le `sentiment_analyzer` (tâche simple et répétitive) et pour le `guardrail` (classification binaire) tout en gardant un modèle plus capable pour l'orchestration principale. Ensuite, le cache Redis décrit dans la section stockage évite de réappeler le LLM pour des produits déjà analysés récemment.
...
**Parallélisation** : actuellement, `analyze_sentiment` traite tous les produits en un seul appel LLM batch. Si le volume de produits venait à augmenter significativement, il pourrait être pertinent de découper en plusieurs appels parallèles via `asyncio.gather` pour éviter des contextes trop larges et des timeouts.
...
**A/B testing de prompts** : en traitant les prompts comme on traiterait des hyperparamètres ML — une base de requêtes de référence établie avec le client, sur laquelle on compare différentes versions du system prompt ou du prompt de sentiment, et on évalue sur une métrique définie (score LLM as Judge, taux de validation Pydantic, etc.). Au final, il faut travailler en étroite collaboration avec le client et utiliser son jugement sur la qualité des réponses
...
**Évolution des capacités** : On pourrait ajouter plus d'outils pour rendre l'agent plus performant. Par exemple, un review tool pourrait être intégré en fin de parcours pour que l'agent puisse réviser son travail avec des règles établies. Il faudrait aussi générer le rapport dans une page web interactive avant de l'exporter en PDF. On pourrait permettre à l'utilisateur de parler avec l'agent en live pour faire des modifications. Finalement, intégrer un système de mémoire à l'agent (enregistré dans une BD par utilisateur ou par persona) où l'agent enregistrerait lorsqu'un utilisateur partage une préférence ou quelque chose qui n'a pas marché.
